...
const TeamData = () => {
  ...
  useEffect(() => {
    ...
    if (teamValue) {
      axios.get(`http://localhost:8080/api/v1/pl/team/${teamValue}`)
        .then(response => {
      ...
      axios.get(`http://localhost:8080/api/v1/pl/nation/${nationValue}`)
      .then(response => {
        setPlayerData(response.data);
        setLoading(false);
      })
      ...
      axios.get(`http://localhost:8080/api/v1/pl/position/${positionValue}`)
      .then(response => {
        setPlayerData(response.data);
        setLoading(false);
      })
      ...
      axios.get(`http://localhost:8080/api/v1/pl/player/${nameValue}`)
      .then(response => {
        setPlayerData(response.data);
        setLoading(false);
      })
      ...
      axios.get(`http://localhost:8080/api/v1/pl/search`,{
        params: {
          name: searchValue.name || '',
          team: searchValue.team || '',
          position: searchValue.position || '',
          nation: searchValue.nation || ''
        }
      })
      .then(response => {
        setPlayerData(response.data);
